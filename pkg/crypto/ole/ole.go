...
func KeyGen(attributes []string, userName string, prefix string) ([]byte, error) {
	...
	fmt.Println(command.Stdout.(*bytes.Buffer).String())
	keyContent, nil := getPlainText(userName + ".key")
	return keyContent, nil
}

func Encrypt(data []byte, policy string, prefix string, inputFileName string, outputFileName string) ([]byte, error) {
	...
	fmt.Println(command.Stdout.(*bytes.Buffer).String())
	content, err := getPlainText(outputFileName + ".cpabe")
	return content, nil
}

func Decrypt(ciphertextFileName string, userName string, prefix string, plaintextFileName string) ([]byte, error) {
	//write data into file
	filePath := "./" //os.Executable()
	...
	fmt.Println(command.Stdout.(*bytes.Buffer).String())
	content, err := getPlainText(plaintextFileName)
	return content, nil
}
...
func SaveText(data []byte, outputFileName string) error {
	filePath := "./" //os.Executable()

	_, err := os.Stat(filePath + "/" + outputFileName)
	if !os.IsNotExist(err) {
		err := os.Remove(filePath + "/" + outputFileName)
		if err != nil {
			fmt.Println(err)
			return err
		}
	}
	file, err := os.Create(filePath + "/" + outputFileName)
	if err != nil {
		fmt.Println(err)
		return err
	}
	file, err = os.OpenFile(filePath+"/"+outputFileName, os.O_WRONLY|os.O_CREATE, 0666)
	//close file
	defer func(file *os.File) {
		err := file.Close()
		if err != nil {
